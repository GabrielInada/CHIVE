...
| `localStorage` | `chive.settings` | Device-level settings such as the TIN color-rendering mode. Not part of projects, imports, or exports. |
...
preferences, device-level settings, and the saved language choice for that
...
remove `chive-locale` or `chive.settings`: language and device-level settings
survive clearing the current project.
