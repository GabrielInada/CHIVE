...
- **`no-undef` (error).** Undefined-identifier detection is on. Browser and Web Worker globals are maintained **by hand** in `BROWSER_GLOBALS` ([eslint.config.js](eslint.config.js)), a deliberate choice to avoid the `globals` dependency and stay consistent with the minimal-footprint stance. The first time you use a new browser API (e.g. `navigator`), add it to that list or lint will flag it.
