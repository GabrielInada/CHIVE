...
| **Flux / Redux** (actions, reducers, immutable store) | Pure reducers, time-travel devtools, predictable updates. | Reducer + action ceremony for ~25 mutations is overkill. Immutability fights D3 because charts mutate DOM imperatively, so the store's snapshot purity buys nothing downstream. The async path we have (IndexedDB persistence) is a single wildcard subscriber that debounces saves, not a reducer pipeline plus middleware. | Rejected: ceremony cost outweighs benefit at this surface size. |
...
| **A framework (React / Vue / Svelte)** | Component model, virtual DOM diffing, ecosystem. | D3 + VDOM is a known friction point: escape hatches and refs everywhere, or a rewrite of the chart layer. Adds a heavy build dep and a deep tree of transitive dependencies to a project whose appeal is "open `index.html`, see the app", with a minimal footprint as part of its privacy story. | Rejected: vanilla JS + D3 is a deliberate stack choice; a framework would invert it. |
...
| **Visualization Layer** | Renderers and DOM builders. They read durable app state via getters and derive DOM from it. They do not own durable mutations; user actions are surfaced as callbacks the controller layer injects. D3 visualizations and the dashboard `panelSubsystem/` subsystem (live-mounted slots, snapshot rendering, SVG export) live here. | `components/`, `modules/visualizations/`, `modules/panelSubsystem/` *(except `panelStateMutations.js` and `blockStateHelpers.js`, which back the panel facade and belong to the State Management Core)* |
...
**Visualization Layer** derives DOM from durable app state. Renderers and DOM builders read state via getters and produce DOM; user input is surfaced through callbacks the controller layer injects (e.g. `panelManager` wires DOM event handlers to facade calls and passes the resulting callbacks down into the renderer). Renderers do not emit. Module-local transient UI state is permitted where it belongs to a single dialog or interaction window: the file-list search query and pagination in [resultsView.js](src/components/resultsView.js), the dialog draft rules in [globalFilterDialog.js](src/components/results/globalFilterDialog.js), the sidebar focus anchor in [chartControlsManager.js](src/modules/chartControls/chartControlsManager.js). Durable application state still belongs in `appState`.
...
Three of them, [dataStateFacade.js](src/modules/state/dataStateFacade.js), [uiStateFacade.js](src/modules/state/uiStateFacade.js), and [panelStateFacade.js](src/modules/state/panelStateFacade.js), are composed in [appState.js](src/modules/state/appState.js) and re-exported from there. Every public mutator follows the same shape: validate, write, emit. The emit step uses a `STATE_EVENTS.*` constant; never a string literal.
...
- A 100-entry ring-buffer logger toggleable at runtime via `window.chiveDebug.enableStateLog()`. When enabled, every emission is printed as `[chive:state] <type> <data>` and pushed into the buffer. When disabled, it is a single boolean check with zero overhead.
...
> 5. Renderers produce DOM; D3 charts redraw. None of them emit anything, and the cycle ends.

The punchline: **durable application state goes through facades, which emit on every mutation; the renderers that visualize that state do not own durable mutations.** Transient UI interaction state (a search query, a dialog draft, a sidebar focus anchor) can live module-local in the controller or dialog that owns it. The facade-emit-subscribe cycle is the spine.

Not every event takes the path above. A panel-layout change (e.g. `PANEL_BLOCK_PROPORTIONS_UPDATED`) does **not** route through `main.js`; `panelManager` is the subscriber and it redraws only the panel canvas. The shape is identical: facade emits, subscriber reacts. The subscriber set varies per event, and the response varies from "rebuild the world" (`main.js`) to "redraw the panel" (`panelManager`) to "no subscriber at all" (some UI events are emitted for symmetry but every consumer mutates the DOM synchronously after the facade call).

> Hard rules for contributors live in [CONTRIBUTING.md](CONTRIBUTING.md): invariants, where new code goes, code conventions, and debugging helpers. Read those before opening a PR.
