/**
 * 3D-scatter activation defaults.
 *
 * Computes the column selection applied the first time the 3D scatter is
 * activated for a dataset. Pure: reads `dataset.chartConfig` and the
 * column context, returns a plain config patch, touches no DOM and no
 * state. Mirrors the TIN chart's picker.
 *
 ...
 * Falls back to the first available option, then `null`.
 *
 * @private
...
/**
 * Compute the 3D scatter's activation defaults. Picks the first three
